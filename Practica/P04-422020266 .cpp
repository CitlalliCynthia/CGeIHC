...
int main()
{
	...
	*/
	...
	glm::mat4 modelJerarquia(1.0);//Inicializar matriz de Modelo 4x4 auxiliar para la jerarquía
	...
	while (!mainWindow.getShouldClose())
	{
		...
		modelaux = model; //Arriba va la información que sí voy a heredar
		modelJerarquia = model;	//Arriba va la información que sí voy a heredar
		//Se dibuja el cubo
		model = glm::scale(model, glm::vec3(5.0f, 4.0f, 5.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix()));
		color = glm::vec3(0.98431f, 0.960784f, 0.32549f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo y pirámide triangular

		//Articulación 1
		model = modelaux; //Restaurar a la información a heredar
		model = glm::translate(model, glm::vec3(0.0f, 2.0f, 0.0f));
		//rotación alrededor de la articulación que une con la cabina
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion1()), glm::vec3(0.0f, 0.0f, 1.0f));
		//Arriba va la información que sí voy a heredar
		modelaux = model;
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		//El color lo comparte con la cabina
		sp.render();

		//Restaurar a la información a heredar
		model = modelaux;
		...
		model = modelaux;//descartar los cambios
		...
		model = glm::translate(model, glm::vec3(2.5f, 0.0f, 0.0f));
		modelaux = model; //Arriba va la información que sí voy a heredar
		//Se dibuja el prisma cuadrangular
		model = glm::scale(model, glm::vec3(5.0f, 1.0f, 1.0f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		...
		modelaux = model; //Arriba va la información que sí voy a heredar
		//Se dibuja la esfera (utiliza el mismo color que el brazo)
		model = glm::scale(model, glm::vec3(0.5f, 0.5f, 0.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		sp.render();
		...
		//BASE
		model = modelJerarquia; //Descartar los cambios a la información de la cabina
		model = glm::translate(model, glm::vec3(0.0f, -2.0f, 0.0f));
		modelJerarquia =  model; //Se hereda la información de arriba
		...
		model = modelJerarquia; //Descartar los cambios

		model = glm::translate(model, glm::vec3(2.5f, -1.5f, 3.5f));
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion5()), glm::vec3(0.0f, 1.0f, 0.0f));
		modelaux = model; //Arriba va la información que sí voy a heredar
		//dibujar un pequeño cilindro
		model = glm::scale(model, glm::vec3(0.5f, 1.5f, 0.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.670588f, 0.8588235f, 0.890196f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		...
		model = glm::scale(model, glm::vec3(1.75f, 1.5f, 1.75f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.886274f, 0.529411f, 0.262745f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[2]->RenderMeshGeometry(); //dibuja las figuras geométricas cilindro y cono
		...
		*/
		...
		//CUERPO
		model = glm::translate(model, glm::vec3(0.0f, 7.75f, -4.0f));
		modelaux = model; //Arriba va la información que sí voy a heredar
		modelJerarquia = model;	//Arriba va la información que sí voy a heredar
		//Se dibuja el cubo
		model = glm::scale(model, glm::vec3(5.0f, 3.5f, 3.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		glUniformMatrix4fv(uniformProjection, 1, GL_FALSE, glm::value_ptr(projection));
		glUniformMatrix4fv(uniformView, 1, GL_FALSE, glm::value_ptr(camera.calculateViewMatrix()));
		color = glm::vec3(0.9529411f, 0.666666f, 0.2784313f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo y pirámide triangular
		...
		modelaux = model; //Arriba va la información que sí voy a heredar
		model = glm::scale(model, glm::vec3(2.9f, 2.9f, 3.45f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.9529411f, 0.666666f, 0.2784313f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo y pirámide triangular
		...
		model = glm::scale(model, glm::vec3(1.0f, 1.5f, 3.45f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.8470588f, 0.8274509f, 0.8235294f);
		glUniform3fv(uniformColor, 1, glm::value_ptr(color)); //para cambiar el color del objetos
		meshList[0]->RenderMesh(); //dibuja cubo y pirámide triangularo

		//OREJA 1
		model = modelaux; //Restaurar a la información a heredar
		model = glm::translate(model, glm::vec3(0.8f, 2.33f, 1.1f));
		...
		//PATA1 Y ARTICULACION 1
		model = modelJerarquia; //Restaurar a la información a heredar del cuerpo
		model = glm::translate(model, glm::vec3(1.75f, -1.9f, 1.0f)); //2.75
		model = glm::rotate(model, glm::radians(mainWindow.getarticulacion3()), glm::vec3(0.0f, 0.0f, 1.0f));
		modelaux = model; //Arriba va la información que sí voy a heredar
		//Se dibuja la esfera
		...
		//pata1
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, -0.7f, 0.0f)); //2.75
		modelaux = model; //Arriba va la información que sí voy a heredar
		...
		//PATA1 Y ARTICULACION 2
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, -1.0f, 0.0f)); //-1.0
		...
		//pata1 parte2
		model = modelaux;
		model = glm::translate(model, glm::vec3(0.0f, -1.0f, 0.0f)); //-0.5
		modelaux = model; //Arriba va la información que sí voy a heredar
		model = glm::scale(model, glm::vec3(1.5f, 2.0f, 1.5f));
		glUniformMatrix4fv(uniformModel, 1, GL_FALSE, glm::value_ptr(model));
		color = glm::vec3(0.9529411f, 0.666666f, 0.2784313f);
		...
		//COLA
		model = modelJerarquia; //Restaurar a la información a heredar del cuerpo
		model = glm::translate(model, glm::vec3(-4.5f, 1.0f, 0.0f)); //-0.5
		modelaux = model; //Arriba va la información que sí voy a heredar
		...
		//color = glm::vec3(0.792156f, 0.674509f, 0.5921568f);
		glUseProgram(0);
